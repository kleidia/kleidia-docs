...
fresh_deployment() {
    ...
    sleep 20
    # CNPG pods use cnpg.io/cluster label, not app=postgres
    kubectl wait --for=condition=ready pod -l cnpg.io/cluster=kleidia-db -n "$NAMESPACE" --timeout=300s 2>/dev/null || {
        # Fallback: try legacy label for non-CNPG deployments
