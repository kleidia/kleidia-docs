...
2. **Copy the MSI files** to the share:
   ```powershell
   Copy-Item "kleidia-agent-X.Y.Z-unsigned.msi" "\\DC\Software\Kleidia\"
...
try {
    ...
    $agentMsi = "\\DC\Software\Kleidia\kleidia-agent-X.Y.Z-unsigned.msi"
...
2. **Create a source folder** with all required files:
   ```
   C:\Kleidia-Intune\
   ├── kleidia-agent-X.Y.Z-unsigned.msi
   ├── yubikey-manager.msi
   ├── agent.toml
   └── install.ps1
   ```
...
3. **Create install.ps1**:
   ...
   # Install Kleidia Agent
   Start-Process msiexec.exe -ArgumentList "/i `"$scriptDir\kleidia-agent-X.Y.Z-unsigned.msi`" /qn /norestart" -Wait -NoNewWindow
...
2. **Create install.cmd**:
   ...
   msiexec.exe /i "%~dp0kleidia-agent-X.Y.Z-unsigned.msi" /qn /norestart /L*v "%TEMP%\kleidia-install.log"
   EXIT /B %ERRORLEVEL%
   ```
...
3. **Create uninstall.cmd**:
   ...
   msiexec.exe /x "%~dp0kleidia-agent-X.Y.Z-unsigned.msi" /qn /norestart
   EXIT /B %ERRORLEVEL%
   ```
...
4. **Add Deployment Type**:
   ...
   - **Content location**: `\\SCCM\Source$\Kleidia\X.Y.Z\`
...
msiexec /i kleidia-agent-X.Y.Z-unsigned.msi /qn

# Install with backend URL
msiexec /i kleidia-agent-X.Y.Z-unsigned.msi /qn BACKEND_URL=https://kleidia.example.com

# Install with logging
msiexec /i kleidia-agent-X.Y.Z-unsigned.msi /qn /L*v C:\install.log

# Install without restart
msiexec /i kleidia-agent-X.Y.Z-unsigned.msi /qn /norestart
```
...
# Or by MSI file
msiexec /x kleidia-agent-X.Y.Z-unsigned.msi /qn
...
- **Version**: Agent version (e.g., `X.Y.Z`)
